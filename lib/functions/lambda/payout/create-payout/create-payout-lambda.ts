...
function parseBody(event: { body?: string | null }): { amount: number; payoutMethod: string } | null {
  ...
  try {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || !payoutMethod) return null;
    return { amount, payoutMethod };
  } catch {
    return null;
  }
}
...
export const handler = async (event: ApiGatewayEvent) => {
  initTelemetryLogger(event, { domain: "payout-domain", service: "create-payout" });
  ...
  if (!body) {
    return {
      statusCode: 400,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: 'Invalid body: amount (number > 0) and payoutMethod (string) required' }),
    };
  }
